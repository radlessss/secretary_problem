import unittest
from secretary_package.threshold_agent import CooperativeTwoSideThresholdAgent, FixedThresholdStrategyAgent
...
class TestThresholdAgent(unittest.TestCase):
    ...
    def test_FixedThresholdStrategyAgent(self):
        agent = FixedThresholdStrategyAgent(threshold=0.2, count_of_candidates=10)

        action = agent.make_decision([[1, 0, 0.3]])
        self.assertEqual(action, 0)
        self.assertEqual(agent.max_obs, 0.3)

        action = agent.make_decision([[2, 0, 0.5]])
        self.assertEqual(action, 0)
        self.assertEqual(agent.max_obs, 0.5)

        action = agent.make_decision([[3, 0, 0.7]])
        self.assertEqual(action, 1)
        self.assertEqual(agent.max_obs, 0.7)
