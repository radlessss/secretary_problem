# secretary_problem
## Посилання на оригінальну задачу: https://github.com/nima-siboni/secretary-problem-RL-DDQN.git
...
**1.Ймовірність того, що обраний кандидат є абсолютним максимумом усій вибірці (не у всіх симуляціях відбувається шлюб, проте якщо шлюб відбувся, то це означає, що знайшлася пара, яка краща за ту, що була у фазі спостереження)**
...
**Відносний ранг** — це місце кандидата серед уже переглянутих. Під час інтерв’ю ми не знаємо, хто прийде далі, тому можемо лише порівняти поточного кандидата з попередніми: якщо він кращий за всіх, кого бачили, його відносний ранг дорівнює 1.

**Абсолютний ранг** — це реальне місце кандидата серед усіх N кандидатів. Його можна визначити лише після завершення відбору, коли відомі якості всіх. 

  - `r_i` - абсолютний ранг кандидата, якого обрав алгоритм у i-й симуляції  
  - `n_total` - загальна кількість симуляцій 
...
$$
   ...
   Наприклад, якщо обрані кандидати мали абсолютні ранги `r = [2, 5, 1, 3, 4]` у 5 симуляціях:
